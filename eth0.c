...
uint32_t sum;
uint8_t macAddress[HW_ADD_LENGTH] = {2,3,4,5,6,131};
...
uint8_t ipDNS[IP_ADD_LENGTH] = {0, 0, 0, 0};
...
// Sets DNS
void etherSetDns(uint8_t mask0, uint8_t mask1, uint8_t mask2, uint8_t mask3)
{
    ipDNS[0] = mask0;
    ipDNS[1] = mask1;
    ipDNS[2] = mask2;
...
// Gets DNS
void etherGetDns(uint8_t dns[4])
{
    uint8_t i;
    for (i = 0; i < 4; i++)
        dns[i] = ipDNS[i];
